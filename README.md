...
## v67 Admin Student Workspace
- Redesigned the Admin Portal Students tab with a dedicated dark cyan/blue workspace theme, compact metrics, focused filters, student identity cells, and clearer action grouping.

## v69 payment function configuration

`submit-payment` uses Supabase-provided Edge Function environment variables `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`. These must not be created as custom secrets with the `SUPABASE_` prefix.

The only custom secret required for the payment function is:

- `TURNSTILE_SECRET_KEY`

Keep Edge Function JWT verification disabled for this public payment submission endpoint. Deploy/redeploy the `submit-payment` function after confirming the custom secret is present.

## v70 payment deployment

`submit-payment` is configured with `verify_jwt = false` in `supabase/config.toml` because public enrolment submissions do not require a Supabase user session. Security is provided by Turnstile, server-side course/amount validation, duplicate-UTR protection, rate limiting, and server-side creation of PENDING payments.

Supabase-managed Edge Function environment variables are used for `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`; do not create custom secrets with the `SUPABASE_` prefix. Add only the custom `TURNSTILE_SECRET_KEY` secret.

After deployment, opening the function URL with GET should return JSON containing `ok: true`, `service: submit-payment`, and `function_version: v70`. This is a deployment check only; payment creation remains POST-only.

## v71 payment button validation
- The public UPI payment button now derives its enabled state from all required fields plus a live Turnstile token.
- Full and 2-Part payment options use the same readiness validation.
- Turnstile is explicitly rendered when the payment modal opens, avoiding hidden-modal/auto-render timing issues.
- Payment submission remains disabled while the request is in progress and after a successful pending submission.
