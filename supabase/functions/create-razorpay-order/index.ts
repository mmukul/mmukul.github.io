...
async function razor(path:string, init:RequestInit){
  ...
  let r; try { r=await fetch(`https://api.razorpay.com/v1${path}`,{...init,signal:controller.signal,headers:{Authorization:`Basic ${auth}`,'Content-Type':'application/json',...(init.headers||{})}}); } finally { clearTimeout(timer); }
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(!supabaseUrl||!serviceRole||!Deno.env.get('RAZORPAY_KEY_ID')||!Deno.env.get('RAZORPAY_KEY_SECRET')) return json(req,500,{error:'Payment service is not configured. Add the Razorpay server secrets in Supabase Edge Functions.'});
  ...
  }catch(e){console.error('create-razorpay-order',e);const raw=String(e?.message||'');let m='Unable to start secure payment. Please try again.';if(e?.name==='AbortError')m='Razorpay gateway timed out. Please try again.';else if(/Payment service is not configured/i.test(raw))m=raw;else if(/payment_catalog|gateway_payment_orders|relation .* does not exist|column .* does not exist/i.test(raw))m='Payment database setup is incomplete. Run the Razorpay payment SQL migration in Supabase.';else if(/Razorpay request failed/i.test(raw)||/authentication failed|key_id|secret/i.test(raw))m='Razorpay server credentials are invalid or not configured.';return json(req,502,{error:m,error_code:'GATEWAY_ORDER_ERROR'});}
});
