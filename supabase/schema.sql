...
-- Admin-only course assignment/update RPC.
...
begin
  if not public.is_admin() then
    raise exception 'Only active administrators can assign courses.';
  end if;
  ...
  if not exists (
    ...
    where id=target_student_id and role='student' and status='active'
  ) then
    raise exception 'Target account is not an active student.';
  end if;
  ...
  if target_batch_id is not null and not exists (
    ...
    where id=target_batch_id
      and (course_id=target_course_id or course_id is null)
  ) then
