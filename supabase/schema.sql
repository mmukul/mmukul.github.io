...
-- Admin-only enrollment assignment/update. This is used by the Admin Portal
-- because direct browser writes to enrollments are protected by RLS.
...
begin
  if not public.is_admin() then
    raise exception 'Only active administrators can assign courses.' using errcode = '42501';
  end if;

  if target_status not in ('active','completed','cancelled') then
    raise exception 'Invalid enrollment status: %', target_status using errcode = '22023';
  end if;

  if not exists (select 1 from public.profiles where id = target_student_id and role = 'student') then
    raise exception 'Target user is not a student profile.' using errcode = '22023';
  end if;
