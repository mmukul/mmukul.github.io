...
-- Existing deployments may already have the older recordings column names.
-- Add the canonical columns and migrate legacy values without deleting data.
...
-- ============================================================
-- 10. Student Lab Assignments
-- ============================================================
-- Canonical lab assignment table used by both Admin Portal and Student Portal.
...
create index if not exists idx_lab_assignments_student on public.lab_assignments(student_id);
create index if not exists idx_lab_assignments_status on public.lab_assignments(status);
...
create policy "students read own lab assignments"
on public.lab_assignments for select to authenticated
using (student_id = auth.uid() and public.is_active_student());

create policy "admins manage lab assignments"
on public.lab_assignments for all to authenticated
using (public.is_admin())
with check (public.is_admin());

grant select on public.lab_assignments to authenticated;
grant insert, update, delete on public.lab_assignments to authenticated;

-- Legacy compatibility table retained for older deployments.
-- New Admin/Student code uses public.lab_assignments above.

create table if not exists public.student_lab_assignments (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references public.profiles(id) on delete cascade,
  lab_type text not null check (lab_type in ('free','aws')),
  status text not null default 'active' check (status in ('active','inactive','expired')),
  assigned_at timestamptz not null default now(),
  expires_at timestamptz,
  created_at timestamptz not null default now(),
  unique(student_id, lab_type)
);

alter table public.student_lab_assignments enable row level security;

drop policy if exists "students read own lab assignments" on public.student_lab_assignments;
create policy "students read own lab assignments"
on public.student_lab_assignments for select to authenticated
using (student_id = auth.uid() and public.is_active_student());

drop policy if exists "admins manage lab assignments" on public.student_lab_assignments;
create policy "admins manage lab assignments"
on public.student_lab_assignments for all to authenticated
using (public.is_admin())
with check (public.is_admin());

grant select, insert, update, delete on public.student_lab_assignments to authenticated;
