...
\n\n-- ============================================================\n-- 6A. Secure course assignment and student course retrieval RPCs\n-- ============================================================\n\n-- Admin-only enrollment write. The browser must not INSERT directly into\n-- enrollments because the table is protected by RLS.\ncreate or replace function public.admin_assign_course(\n  target_batch_id uuid,\n  target_course_id uuid,\n  target_status text,\n  target_student_id uuid\n)\nreturns public.enrollments\nlanguage plpgsql\nsecurity definer\nset search_path = public\nas $$\ndeclare\n  result public.enrollments;\nbegin\n  if not public.is_admin() then\n    raise exception 'Admin access required';\n  end if;\n\n  if target_status not in ('active','completed','cancelled') then\n    raise exception 'Invalid enrollment status: %', target_status;\n  end if;\n\n  if not exists (select 1 from public.profiles where id=target_student_id and role='student') then\n    raise exception 'Target user is not a student';\n  end if;\n\n  if not exists (select 1 from public.courses where id=target_course_id) then\n    raise exception 'Course not found';\n  end if;\n\n  if target_batch_id is not null and not exists (\n    select 1 from public.batches where id=target_batch_id and course_id=target_course_id\n  ) then\n    raise exception 'Selected batch does not belong to the selected course';\n  end if;\n\n  -- Update the newest existing enrollment for this student/course.\n  select * into result\n  from public.enrollments\n  where student_id=target_student_id and course_id=target_course_id\n  order by enrolled_at desc\n  limit 1;\n\n  if result.id is not null then\n    update public.enrollments\n    set batch_id=target_batch_id, status=target_status\n    where id=result.id\n    returning * into result;\n  else\n    insert into public.enrollments(student_id,course_id,batch_id,status)\n    values(target_student_id,target_course_id,target_batch_id,target_status)\n    returning * into result;\n  end if;\n\n  return result;\nend;\n$$;\n\nrevoke all on function public.admin_assign_course(uuid,uuid,text,uuid) from public;\ngrant execute on function public.admin_assign_course(uuid,uuid,text,uuid) to authenticated;\n\n-- Student course list. This avoids a fragile nested PostgREST relationship/RLS\n-- query while still enforcing that only an active student can see their own\n-- active/completed enrollments.\ncreate or replace function public.student_list_my_courses()\nreturns table (\n  enrollment_id uuid,\n  course_id uuid,\n  course_name text,\n  course_slug text,\n  course_description text,\n  batch_id uuid,\n  batch_name text,\n  status text,\n  enrolled_at timestamptz\n)\nlanguage sql\nstable\nsecurity definer\nset search_path = public\nas $$\n  select e.id, c.id, c.name, c.slug, c.description,\n         e.batch_id, b.name, e.status, e.enrolled_at\n  from public.enrollments e\n  join public.courses c on c.id=e.course_id\n  left join public.batches b on b.id=e.batch_id\n  where e.student_id=auth.uid()\n    and e.status in ('active','completed')\n    and public.is_active_student()\n  order by e.enrolled_at desc;\n$$;\n\nrevoke all on function public.student_list_my_courses() from public;\ngrant execute on function public.student_list_my_courses() to authenticated;\n
-- ============================================================
