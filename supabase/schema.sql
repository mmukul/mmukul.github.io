...
-- Allow the authenticated user to create only their own profile if needed.
create policy "students insert own profile" on public.profiles
for insert with check (id = auth.uid());

-- Course visibility can be restricted further later; the portal currently uses enrollment for personalization.

-- =========================================================
-- Admin Portal security extension
-- =========================================================
-- Run this section after the original schema above.
-- IMPORTANT: create the first admin manually in SQL; public signup can never create an admin.
...
-- Admin access to LMS management tables.
...
-- First-admin bootstrap example (replace with the UUID of an existing auth user):
